    ...
    pc = try( 
      {
        ...
        veMat = wgcnaCor(svd1$v[, c(1:min(n,p,nVarExplained))], t(datModule), use = "p") 
    ...
    if (class(pc)=='try-error')
    {
      ...
      if (subHubs)
      {
        ...
        pc = try( 
        {
          ...
          varExpl[1, i] = mean(wgcnaCor(pcx, t(datModule), use = "p")^2, na.rm = TRUE)
          pcx
        }, silent = TRUE);
      }
    }
    ...
    if (class(pc)=='try-error')
    {
      ...
      ae = try( 
      {
        ...
        if (align == "along average")
        {
          ...
          corAve = wgcnaCor(averExpr[,i], PrinComps[,i], use = "p");
  ...
  if ("eigengenes" %in% names(MEs))
  {
     if (is.null(order))
     {
       if (verbose>0) printFlush(paste(spaces, "orderMEs: order not given, calculating using given set", 
                                          orderBy));
       corPC = wgcnaCor(MEs$eigengenes, use="p")
       disPC = 1-corPC;
     ...
     if (is.null(order))
     {
       if (verbose>0) printFlush(paste(spaces, "orderMEs: order not given, calculating using given set", 
                                          orderBy));
       corPC = wgcnaCor(MEs[[orderBy]]$data, use="p")
       disPC = 1-corPC;
       order = .clustOrder(disPC, greyLast = greyLast, greyName = greyName);
     } 
...
consensusMEDissimilarity = function(MEs, useAbs = FALSE, useSets = NULL, method = "consensus")
{
  ...
  for (set in useSets)
  {
    if (useAbs)
    {
        diss = 1-abs(wgcnaCor(MEs[[set]]$data, use="p"));
    } else
    {
        diss = 1-wgcnaCor(MEs[[set]]$data, use="p");
    }
  ...
  if (intType < 4)
  {
    if (is.null(selectCols))
    {
      if (is.list(corOptions))
      {
         ...
         # cor_mat = wgcnaCor(datExpr, use = "p");
      ...
      if (is.list(corOptions))
      {
        ...
        corExpr = parse(text = paste(corFnc, "(datExpr, datExpr[, selectCols] ", prepComma(weightOpt), 
                                     prepComma(corOptions), ")"));
        #cor_mat = wgcnaCor(datExpr, datExpr[, selectCols], use="p");
        cor_mat = eval(corExpr);
      }
    }
  } else {
                             ...
                             corFnc = "cor", corOptions = "use = 'p'")
{
  ...
  # abs(wgcnaCor(datExpr, datExpr2, use="p"))^power;
                     ...
                     corFnc = "cor", corOptions = "use = 'p'") 
{
  ...
  #output=data.frame(wgcnaCor(datExpr, datME, use="p"))
                             ...
                             ...) 
{
  ...
  #cor=signif(wgcnaCor(x,y,use="p",method=correlationmethod),2)
...
plotMEpairs=function(datME, y=NULL, main="Relationship between module eigengenes", clusterMEs=TRUE, ...)
{
  if ( dim(as.matrix(datME))[[2]]==1 & is.null(y) ) 
  {
    ...
    {
      dissimME=(1-t(wgcnaCor(datME, method="p", use="p")))/2
...
relativeCorPredictionSuccess=function(corPredictionNew, corPredictionStandard, corTestSet, topNumber=100 )
{
  ...
  for (i in c(1:nPredictors) )
  {
    ...
    for (j in c(1:length(topNumber)) ){
      ...
      sign1=sign(wgcnaCor(Grouping,x1, use="p"))
      if (sign1==0) sign1=1
      ...
      #print(paste("This correlation is positive if the new method is better than the old method" , 
                   # signif(wgcnaCor(Grouping,x1, use="p"),3)))
    } # end of j loop
  } # end of i loop
...
alignExpr=function(datExpr, y = NULL) 
{
  ...
  sign1=sign(as.numeric(wgcnaCor(y, datExpr, use="p" )))
       ...
       getQValues = TRUE, ...) 
{
  ...
  ES=data.frame(wgcnaCor(y, datME, use="p"))
               ...
               getQValues = TRUE)
{
  ...
  #Cor.Standard= as.numeric(wgcnaCor(y,datExpr,use= "p") )
  ...
  if (nGenes==1) 
  {
    #RawCor.Weighted=as.numeric(wgcnaCor(y,datExpr,use= "p") )
    corExpr = parse(text = paste("as.numeric(" , corFnc, "(y,datExpr ", prepComma(corOptions), "))"));
  ...
  while (start <= nMEs)
  {
    ...
    datKMEBatch=as.matrix(signedKME(datExpr,datMEBatch, outputColumnName="MM.", 
                                    corFnc = corFnc, corOptions = corOptions))
    # ES.CorBatch= as.vector(wgcnaCor(  as.numeric(as.character(y))  ,datMEBatch, use="p"))
...
correlationPreservation = function(multiME, setLabels, excludeGrey = TRUE, greyLabel = "grey")
{
  ...
  for (i in 1:(nSets-1))
    for (j in (i+1):nSets)
    {
      corME1 = wgcnaCor(multiME[[i]]$data[, Use], use="p");
      corME2 = wgcnaCor(multiME[[j]]$data[, Use], use="p");
                      ...
                      ...)
{
  ...
  if (plotDendrograms) for (set in 1:nSets)
  {
    ...
    corME = wgcnaCor(multiME[[set]]$data[substring(labels,3)!=greyLabel,
  ...
  if (plotHeatmaps) for (i.row in (1:nSets)) for (i.col in (1:nSets))
  {
    ...
    if (i.row==i.col)
    {
      corME = wgcnaCor(multiME[[i.col]]$data, use="p") 
    ...
    {
      corME1 = wgcnaCor(multiME[[i.col]]$data, use="p");
      corME2 = wgcnaCor(multiME[[i.row]]$data, use="p");
...
datExpr=data.frame(datExpr)
    ...
    if (fastCalculation) {
        ...
        corDeviance = as.numeric(wgcnaCor(devianceresidual, datExpr, 
            use = "p"))
