...
networkConcepts = function(datExpr, power=1, trait=NULL, networkType = "unsigned") 
{
  ...
  if(networkTypeC==1)
  {
        adj <- abs(wgcnaCor(datExpr,use="p"))^power
  } else if (networkTypeC==2)
  {
  	adj <- abs((wgcnaCor(datExpr,use="p")+1)/2)^power
  } else {
        cor = wgcnaCor(datExpr,use="p");
        cor[cor < 0] = 0;
  	adj <- cor^power
  }
  ...
  ConformityE=wgcnaCor(datExpr,m1[[1]][,1],use="pairwise.complete.obs"); ConformityE=abs(ConformityE)^power; 
  ...
  if(!is.null(trait)){
    EigengeneSignificance = abs(wgcnaCor(trait, m1[[1]], use="pairwise.complete.obs") )^power;
    EigengeneSignificance = EigengeneSignificance[1,1]
    GS= abs(wgcnaCor(datExpr, trait, use="pairwise.complete.obs") )^power; GS=GS[,1]
